...
class _ShoppingCartState extends State<ShoppingCart>{
  ...
  void fetchSavedItems() async {
    Map<String,List<String>> combined = {};
    int count = 0;
    var ref = Firestore.instance.collection("insta_items")
        .document(currentUserModel.id).collection("items");
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        leading: IconButton(
          icon: Icon(OMIcons.close,color: Colors.black,),
          onPressed: () => {
      ...
      body: savedItems.isNotEmpty ? Column(
        mainAxisSize: MainAxisSize.min,
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Padding(
            padding: const EdgeInsets.only(top: 8.0, bottom: 4.0, left: 16.0),
            child: Text("$itemCount items from ${savedItems.length} ${savedItems.length == 1 ? "shop" : "shops"}"),
          ),
          Padding(
